...
!=======================================================================
  ...
  ! constants
  !
  integer,      parameter, public :: CondTypeDISTANCE    = 1  
  integer,      parameter, public :: CondTypeBANGLE      = 2  ! Not supported yet
  integer,      parameter, public :: CondTypeDANGLE      = 3  ! Not supported yet
  integer,      parameter, public :: CondTypePOS         = 4
  character(*), parameter, public :: CondType(4)         = (/'DISTANCE  ',&
                                                             'BANGLE    ',&
                                                             'DANGLE    ',&
                                                             'POS       '/)
  ! structures
  !
  ! belong to s_option defied below 
  type :: s_option_sldist
    integer :: target_sel = 0
    logical :: only_intra = .false.
    real(8) :: range(2)
  end type

  ! belong to s_option defined below
  type :: s_option_slpos
    real(8) :: range(2,3)
    logical :: use_symmetric = .false.
  end type

  type :: s_option
    integer :: mode(2)   = (/CoMModeRESIDUE, CoMModeRESIDUE/)
    integer :: cond_type = CondTypeDISTANCE 

    type(s_option_sldist) :: sldist
    type(s_option_slpos)  :: slpos
  end type s_option
...
!
      ...
      character(len=MaxChar) :: cond_type       = 'DISTANCE'
