...
!$    write(iw,'("Analyze> OpenMP parallelization is activated")')
      ...
      if (trim(input%flist_weight) /= "") then
        ...
        if (option%use_bootstrap) then
          write(iw,'("Error. Sorry, weight average is not implemented for bootstrap.")')
          stop
        end if
      ...
      if (option%use_bootstrap) then
        ...
        do ifile = 1, cvinfo%nfile
          do istep = option%nsta, cv(ifile)%nstep
            ...
            pop_f      = pop_f      + cv(ifile)%weight(istep)  
            if (ig >= 0 .and. ig <= ngrid) then
              !rdf(ig) = rdf(ig) + 1.0d0
              rdf(ig) = rdf(ig) + cv(ifile)%weight(istep)  
              if (state(ifile)%data(istep) == REACTIVE) then
                !rrdf(ig) = rrdf(ig) + 1.0d0
                !pop_b    = pop_b    + 1.0d0
                rrdf(ig) = rrdf(ig) + cv(ifile)%weight(istep)
                pop_b    = pop_b    + cv(ifile)%weight(istep)
              end if
            end if
          end do
        end do
