...
!-----------------------------------------------------------------------
    subroutine get_state_full(option, cvinfo, cv, state)
!-----------------------------------------------------------------------
      implicit none
      ...
      integer :: ifile, imol
      integer :: nstate, ndim, nfile, nmol, nstep
      ...
      do ifile = 1, nfile
        nstep  = cv%nstep
        allocate(state(ifile)%quench_step(nmol))
        allocate(state(ifile)%data(nstep, nmol))
        allocate(state(ifile)%data_ts(nstep, nmol))
        allocate(state(ifile)%avedata(nstep, nmol))
        allocate(state(ifile)%is_reacted(nmol))
      end do

      if (nmol > nfile) then
        do ifile = 1, nfile
          do imol = 1, nmol
            call get_state_each_mol(               &
              state(ifile)%quench_step(imol),      &
              state(ifile)%data       (:, imol),   &
              state(ifile)%data_ts    (:, imol),   &
              state(ifile)%avedata    (:, imol),   &
              state(ifile)%is_reacted(:),          &
              )
          end do
        end do
...
!-----------------------------------------------------------------------
      ...
      integer                :: istep, istate, imol, icv, imc, ia
      ...
      ! for global
      allocate(state%quench_step(nmol))
