...
contains
  ...
  !
  subroutine enginit
    use mod_output, only : s_output       ! ANATRA str
    use mod_util,   only : open_file,   & ! ANATRA sbr
                           gen_file_name  ! ANATRA sbr
    use mod_anatra_ermod, only : s_option ! ANATRA str
    ...
    use engmain, only: numtype, nummol, engdiv, corrcal, selfcal, slttype, &
         ...
         ene_confname, io_paramfile, io_flcuv, io_popul,     &
         ecdinfo_file, ecdinfo_io, ecdmesh_file, ecdmesh_io, &
         SLT_SOLN, SLT_REFS_RIGID, SLT_REFS_FLEX, PT_SOLVENT, NO, YES, &
         ermax_limit, &
         use_eror, use_anatra, calc_energy, anatra_output, option ! ANATRA
    ...
    integer, parameter :: rglmax = 5, large = 100000
...
3209      continue
    ...
    if(slttype == SLT_SOLN) then
       open(unit = io_paramfile, file = ene_confname, action = "read", iostat = param_err)
       if(param_err == 0 .and. use_eror) then
          read(io_paramfile, nml = eror)
          close(io_paramfile)
       endif
       if(use_eror) then
    ...
    if(myrank == 0) then
       ...
       if (.not. calc_energy) then ! ANATRA
         !call gen_file_name(anatra_output%fhead, 'rcts', fname)
         !open(unit = 15, file = trim(fname), status = 'new', action = 'write')
         !if (option%nstate /= 1) then 
           call gen_file_name(anatra_output%fhead, 'popul', fname)
           open(unit = io_popul, file = trim(fname), status = 'new', action = 'write')
         !end if
       end if
    endif
  ...
  subroutine engclear
    use engmain, only: corrcal, selfcal, slttype, SLT_SOLN, YES, &
                       edens, ecorr, eself, slnuv, avslf, engnorm, engsmpl, use_eror
    implicit none
    edens(:) = 0.0
  ...
  subroutine engproc_cleanup
    use mod_anatra_ermod, only : s_option ! ANATRA str
    use engmain, only: io_flcuv, io_popul, &
                       calc_energy, option, state_count ! ANATRA
    use mpiproc
    !use mpiproc, only: myrank
    implicit none

    integer :: istate, nstate
    real, allocatable :: state_count_r(:) 
    ...
    if (.not. calc_energy) then
      ...
      allocate(state_count_r(0:option%nstate))

      do istate = 0, option%nstate
        state_count_r(istate) = real(state_count(istate))
      end do

      call mympi_reduce_real_array(state_count_r, nstate + 1, mpi_sum, 0)

      !call mpi_gather(state_count(0:nstate),                  nstate + 1, mpi_integer, &
      !                state_count_g(0:nstate, 1:nactiveproc), nstate + 1, mpi_integer, &
      !                0, mpi_comm_activeprocs, ierror) 
      !call mpi_gather(state_count,   nstate + 1, mpi_integer, &
      !                state_count_g, nstate + 1, mpi_integer, &
      !                0, mpi_comm_world, ierror) 

      if (myrank == 0) then
        do istate = 0, option%nstate
          write(io_popul,'(f20.5)') state_count_r(istate)
        end do
     
        close(io_popul)
      endif

      deallocate(state_count_r)
    ...
    if(myrank == 0) then
      if (.not. calc_energy) then
        do istate = 0, option%nstate
          write(io_popul,'(i0)') state_count(istate) 
        end do
  ...
  subroutine engconst(stnum)
    ...
    if(myrank < nactiveproc) then
       ...
       ! cntdst is the iteration no. of insertion (refs)
         ...
         do cntdst = 1, maxdst
           call gen_snapshot(stnum, stat_weight_solute, uvengy(0:slvmax), skipcond)
         enddo
       end if
...
912                   format(2i9, 9999f15.5)
  ...
  subroutine engstore(stnum)
    ...
    use engmain, only: maxcnf, skpcnf, engdiv, corrcal, selfcal, &
         ...
         engnorm, engsmpl, voffset, voffset_initialized, use_eror, &
    ...
    if(use_eror) then
       ...
       if(myrank == 0) then
          ...
          if(division == engdiv) then
             ...
             write(prb_io, '(a,f12.6)') ' Probability to find the system in the OR state = ', factor
             write(prb_io, '(a)') ' Free-energy change from the OR state to the simulated state'
...
751          format(i5, 9999f15.5)
  ...
  subroutine get_uv_energy(stnum, stat_weight_solute, uvengy, out_of_range)
    use engmain, only: maxcnf, skpcnf, slttype, sltlist, cltype,    &
                       ...
                       SLT_SOLN, SLT_REFS_RIGID, SLT_REFS_FLEX,     &
                       use_energy_criteria, energy_min, energy_max, & ! ANATRA
                       energy_criteria_slvind                         ! ANATRA
    ...
    case(SLT_REFS_RIGID, SLT_REFS_FLEX)
       ...
       initialized = .true.
       call instslt('proc', cntdst, stat_weight_solute, out_of_range)
    ...
    do k = 1, slvmax
       ...
       if(use_energy_criteria .and. k ==energy_criteria_slvind)then
         if( uvengy(k) >= energy_min .and. uvengy(k) <= energy_max )then
           out_of_range = .false.
         else
           out_of_range = .true.
         endif
  ...
  subroutine gen_snapshot(stnum, stat_weight_solute, uvengy, out_of_range)
    use engmain, only: maxcnf, skpcnf, slttype, sltlist, cltype, &
                       EL_COULOMB, EL_PME, EL_PPPM, &
                       SLT_SOLN, SLT_REFS_RIGID, SLT_REFS_FLEX
    use ptinsrt, only: instslt
  ...
  subroutine update_histogram(stat_weight_solute, uvengy)
    use engmain, only: wgtslf, estype, slttype, corrcal, selfcal, ermax, &
                       ...
                       voffset, voffset_initialized, use_eror, &
                       SLT_SOLN, SLT_REFS_RIGID, SLT_REFS_FLEX, &
  ...
  subroutine check_mol_configuration(out_of_range)
    use engmain, only: insposition, insstructure, lwreg, upreg, lwstr, upstr, &
                       ...
                       use_selection, stat_cv_system, lwrc, uprc,  &  ! ANATRA
                       SYS_NONPERIODIC, &
    ...
    case(INSPOS_RANDOM)                               ! random
       ...
       if (use_selection) then
         if (stat_cv_system < lwrc .or. stat_cv_system > uprc) then
           out_of_range = .true.
           return
         end if
       end if
