  ...
  subroutine defcond
    ...
    if(clcond == 'merge') then
       ...
       if (slndigit == 2) then
         ...
         opnfile = trim(solndirec) // '/' // trim(slndnspf) // '.001'
       else if (slndigit == 4) then
         opnfile = trim(solndirec) // '/' // trim(slndnspf) // '.0001'
       end if
    else
       opnfile = engfile(1)
    endif
...
729       close(72)
    ...
    if((clcond == 'merge') .and. (readwgtfl == 'yes')) then
       ...
       do i = 1, maxsln
          read(81, *) k, wgtsln(i)
          !!!! KKK !!!!
          write(6,'(i5,2x,e15.7)') i, wgtsln(i)
          !!!!!!!!!!!!!!
       end do
       close(81)
    endif
  ...
  subroutine datread(cntrun)
    ...
    do cnt = 1, 4
       ...
       if(cnt <= 2) then                         ! solution
          ...
          !!!! KKK
          !write(6,'("ECmin,max = ", i5,i5)') ecmin, ecmax
          !write(6,'("Factor = ", e15.7)') factor
          !do pti = ecmin, ecmax 
          !  write(6,'(i5,2x,e15.7)') pti, wgtsln(pti)
          !end do 
          !!!! KKK
       endif
       ...
       do i = ecmin, ecmax
          ...
          case('merge')
             ...
             if (cnt == 1 .or. cnt == 2) then
               if (slndigit == 2) then
                 write(suffnum,'(".",i2.2)') i
               else if (slndigit == 3) then
                 write(suffnum,'(".",i3.3)') i
               else if (slndigit == 4) then
                 write(suffnum,'(".",i4.4)') i
               end if
             else if (cnt == 3 .or. cnt == 4) then
               if (refdigit == 2) then
                 write(suffnum,'(".",i2.2)') i
...
!
    if((uvread /= 'not') .and. (clcond == 'merge')) then
       !!!! KKK
       !do pti = slnini, slnfin
       !  write(6,'(i5,e15.7)') pti, wgtsln(pti)
       !end do
       !!!! KKK
       do pti = 1, numslv
