...
contains
  ...
  ! <--- ANATRA
  subroutine opentrj(ftrj)
    use trajectory, only: open_trajectory

    character(len=*), optional, intent(in) :: ftrj


    if (present(ftrj)) then
      call open_trajectory(history_trajectory, ftrj)
    else
      call open_trajectory(history_trajectory, trjfile)
    end if
  ...
  ! <--- ANATRA
  subroutine OUTconfig(OUTpos, OUTcell, OUTatm, OUTbox, particle_type, calc_type, itrj, readcv)
  ! --->
    use engmain, only : use_selection, ntrj, ftrj, fcv, fcvread_io  ! ANATRA
    ...
    integer, optional, intent(inout) :: itrj   ! ANATRA
    real(8), optional, intent(out)   :: readcv ! ANATRA
    integer :: status
    real    :: dum 
    !
    ...
    case('system')                  ! reading the HISTORY file
       ...
       if (status /= 0) then
         ...
         if (present(itrj)) then
           if (itrj == ntrj) then
             call halt_with_error("set_trj")
           else
             itrj = itrj + 1
             call close_trajectory(history_trajectory)
             call open_trajectory(history_trajectory, trim(ftrj(itrj)))
             call read_trajectory(history_trajectory, OUTatm, (OUTbox == 1), OUTpos, OUTcell, status)
             if (status /= 0) &
               call halt_with_error("set_trj")
             ...
             if (use_selection) then
               close(fcvread_io)
               open(fcvread_io, file=trim(fcv(itrj)), status='old')
               read(fcvread_io,*) dum, readcv 
             end if
...
!
  subroutine iniparam
    ...
    use engmain, only: init_params,                                    &
         ...
         setup_anatra,                                                 & ! ANATRA sbr
         trajopt, traj,                                                & ! ANATRA var
         use_multiple_trj, use_selection, ftrjlist, fcvlist, ntrj,     & ! ANATRA var
         use_eror, ftrj, fcv, lwrc, uprc, anatra_pot_type,             & ! ANATRA var
         maxtrial, stop_fail_ins,                                      & ! ANATRA var
         NO, YES,                                                      &
         ...
         INSORG_ANATRA, INSORG_ANATRA_RANDOM,                          & ! ANATRA
         INSPOS_RANDOM, INSPOS_NOCHANGE, INSPOS_SPHERE,                &
         INSPOS_SLAB_GENERIC, INSPOS_SLAB_SYMMETRIC, INSPOS_RMSD,      &
         INSPOS_GAUSS,                                                 &
         INSPOS_ANATRA, INSPOS_ANATRA_RANDOM,                          & ! ANATRA
         INSROT_RANDOM, INSROT_NOCHANGE,                               &
         INSSTR_NOREJECT, INSSTR_RMSD,                                 &
         ftrj_io, fcv_io,                                              & ! ANATRA
         ANATRA_POT_AMBER, ANATRA_POT_CHARMM                             ! ANATRA
    ...
    use mpiproc, only: myrank, halt_with_error                               ! MPI
    implicit none
    ...
    lwrc             =  0.0
    uprc             = -1.0
    anatra_pot_type  = 0 ! 0:amber-ff, 1:charmm-ff
    maxtrial         = 10
    ! --->
    ...
    ! <--- ANATRA
    if (use_multiple_trj) then
      allocate(ftrj(ntrj))
      open(ftrj_io, file=trim(ftrjlist), status='old')
      do itrj = 1, ntrj
        read(ftrj_io,'(a)') ftrj(itrj)
        !write(6,'("ftrj = ", a)') trim(ftrj(itrj))
      end do
      close(ftrj_io)
    else
      ntrj = 1
    ...
    if (use_selection) then
      ...
      if (lwrc > uprc) then
        stop "lwrc should be smaller than uprc" 
      end if

      allocate(fcv(ntrj))
      open(fcv_io, file=trim(fcvlist), status='old')
      do itrj = 1, ntrj
        read(fcv_io,'(a)') fcv(itrj)
      end do
      close(fcv_io)
    end if
    ...
    ! read ANATRA variables from parameters_er
    if (use_anatra) then
      select case(slttype)
      case(SLT_SOLN)
        stop "ANATRA is not available for SOLN calculation"
      case(SLT_REFS_RIGID, SLT_REFS_FLEX)
        call setup_anatra(calc_energy, myrank)
      case default
        stop "Unknown slttype"
      end select
    end if
    ...
    if (.not. calc_energy) then
      select case(slttype)
      case(SLT_SOLN)
        stop "calc_energy = .true. is not available for SOLN calculation"
      case(SLT_REFS_RIGID, SLT_REFS_FLEX)
        ! do nothing
      case default
        stop "Unknown slttype"
    ...
    case(INSPOS_ANATRA)                               ! ANATRA
       insorigin = INSORG_ANATRA
    case(INSPOS_ANATRA_RANDOM)                        ! ANATRA
       insorigin = INSORG_ANATRA_RANDOM
    case default
    ...
    case(INSORG_ANATRA)
       if(insposition /= INSPOS_ANATRA) &
         call halt_with_error('set_bug')
    case(INSORG_ANATRA_RANDOM)
       if(insposition /= INSPOS_ANATRA_RANDOM) &
         call halt_with_error('set_bug')
    case default
...
! returns number of frames read (EXCLUDING skipped frames)
  ...
  ! <--- ANATRA
  subroutine getconf_parallel(maxread, actual_read, itrj)
  ! --->
    use engmain, only: skpcnf, boxshp, numsite, sluvid, stdout, &
                       sitepos, cell, stat_weight_system,       &
                       perm_file, perm_io,                      &
                       use_multiple_trj, use_selection,         & ! ANATRA
                       stat_cv_system,                          & ! ANATRA
...
99        close(perm_io)
    ...
    if(myrank < nread) then
       if(myrank == 0) then              ! rank-0 to read from file
          ...
          do iproc = 1, nread
             ...
             do i = 1, skpcnf
                ...
                if (use_multiple_trj) then
                  ...
                  if (use_selection) then
                    call OUTconfig(readpos, readcell, OUTatm, boxshp, &
                                                      'system','trjfl_read', itrj = itrj, readcv = readcv) 

                  else
                    call OUTconfig(readpos, readcell, OUTatm, boxshp, &
                                                      'system','trjfl_read', itrj = itrj)
             ...
             if(iproc /= 1) then         ! send the data to other rank
                ...
                ! <--- ANATRA
                call mpi_send(readcv, 1, system_mpikind, &
                              iproc - 1, tag_cv, mpi_comm_world, ierror)
                ! --->
#endif
          ...
          ! <--- ANATRA
          call mpi_recv(cv, 1, system_mpikind, &
                        0, tag_cv, mpi_comm_world, mpistatus, ierror)
          ! --->
#endif
       endif
